...
const Ups = () => {
  ...
  useEffect(() => {
    ...
    filterItems();
  }, [selectedFilter, UPScard]);  // Include UPScard as a dependency
  ...
  return (
    <div className='py-14 sm:min-h-[600px] sm:grid sm:place-items-center bg-gray-100'>
      <div className='container'>
        ...
        <motion.div
          className='cards-container flex justify-center flex-wrap gap-8 py-8'
          ref={ref}
        ...
        >
          {filteredItems.map((item, idx) => (
            <UPSCard
              ...
              onButtonClick={() => handleButtonClick(item.url)}
              key={`ups-card-${idx}`}
            />
          ))}
        </motion.div>
